...
namespace Provider
{
    class EFRole : RoleProvider
    {
        #region members
        private const string EVENTSOURCE = "EFRole";
        ...
        public override bool IsUserInRole(string username, string roleName)
        {
            try
            {
                ...
                {
                    if (!EFMembership.CheckUser(username, ApplicationName, context))
                    {
                        return false;
                    }
        ...
        public override string[] GetUsersInRole(string roleName)
        {
            using (DataManager context = new DataManager("DatabaseEntities"))
            {
                ...
                return role.User.Select(u => u.Username).ToArray();
            }
        }
        ...
        private Role GetRole(Expression<Func<Role, bool>> query, DataManager context)
        {
