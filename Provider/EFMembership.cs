...
namespace Provider
{
    public class EFMembership : MembershipProvider
         {
        #region members
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            minRequiredNonAlphanumericCharacters = Convert.ToInt32(ProviderUtils.GetConfigValue(config, "minRequiredNonAlphanumericCharacters", "0"));
            ...
            requiresQuestionAndAnswer = Convert.ToBoolean(ProviderUtils.GetConfigValue(config, "requiresQuestionAndAnswer", "false"));
                                                  ...
                                                  out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                ...
                using (DataManager context = new DataManager("DatabaseEntities"))
                {
        ...
        /// <returns></returns>
        public static bool CheckUser(string username, string applicationName, DataManager context)
        {
            User user = context.User.FirstOrDefault(u => u.Username == username && u.Application.Name == applicationName);
            if (user == null)
            {
                return false;
            }
        ...
        private User GetUser(Expression<Func<User, bool>> query, DataManager context)
        {
            if (context.User.Where(query).Count() > 0)
            {            
