...
// A class named PersianDateTime which persian developers can use instead of DateTime structure. 
// This class also contains most of the methods and properties of the conventional DateTime structure such as DateTime.Now 
// and DateTime.TimeOfDay.
...
namespace Utility
{
    ...
    public class PersianDateTime
    {
        ...
        private readonly static string[] dayShortName = { "ی", "د", "س", "چ", "پ", "ج", "ش" };
        ...
        public static string ToPersianDate(System.DateTime? dateTime, string format = "{0:f}")
        {
            try
            {
                ...
                format = format.Replace("{0:D}", "{0:dd MMMM، yyyy}");
                format = format.Replace("{0:f}", "{0:dddd، dd MMMM، yyyy}");
                format = format.Replace("{0:F}", "{0:dddd، dd MMMM، yyyy HH:mm tt}");
                ...
                format = format.Replace("MMMM", _monthNames[persianCalendar.GetMonth(dateTime.Value) - 1]);
                format = format.Replace("MMM", _monthNames[persianCalendar.GetMonth(dateTime.Value) - 1]);
                ...
                format = format.Replace("dddd", _dayNames[(int)persianCalendar.GetDayOfWeek(dateTime.Value)]);
