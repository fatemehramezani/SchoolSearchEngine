...
namespace Role.Manager
{
    public partial class School : System.Web.UI.Page
    {
        ...
        protected void SchoolEntityDataSource_Commanding(object sender, Microsoft.AspNet.EntityDataSource.EntityDataSourceChangingEventArgs e)
        {
            ...
            #region File
            string picturePath = Utility.Common.GetConfigValue("SchoolLogoUrl");
            string filePath = Utility.Common.GetConfigValue("SchoolBannerUrl");
            ...
            if (isChanged)
            {
                ...
                {
                    string imgpath = Server.MapPath(picturePath) + "\\" + Profile.ApplicantId;
                    if (!Directory.Exists(imgpath))
                        Directory.CreateDirectory(imgpath);
                    string extension = Path.GetExtension(pictureFileUpload.PostedFile.FileName);
                    string FileName = string.Format("{0}_{1}_عکس_مدرسه_{2}", ((Information)e.Entity).Name.Replace(" ", "_"),Profile.ApplicantId,extension);
            ...
            if (isChanged)
            {
                ...
                {
                    string imgpath = Server.MapPath(filePath) + "\\" + Profile.ApplicantId;

                    if (!Directory.Exists(imgpath))
                        Directory.CreateDirectory(imgpath);

                    string extension = Path.GetExtension(fileFileUpload.PostedFile.FileName);
                    string FileName = string.Format("{0}_{1}_بنر_مدرسه_{2}", ((Information)e.Entity).Name.Replace(" ", "_"), Profile.ApplicantId, extension);
                    string imgFile = imgpath + "\\" + FileName;
                    if (File.Exists(imgFile))
                        File.Delete(imgFile);
